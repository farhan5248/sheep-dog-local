...
public class StepObjectRefFragments {
    ...
    private static final String OBJECT_TYPE = OBJECT_EDGE_TYPE.substring(0, OBJECT_EDGE_TYPE.length() - 1) + "|"
            + OBJECT_VERTEX_TYPE.substring(1);
    private static final String OBJECT = "(" + OBJECT_NAME + OBJECT_TYPE + ")";
    private static final String OBJECT_EDGE = "(" + OBJECT_NAME + OBJECT_EDGE_TYPE + ")";
    private static final String OBJECT_VERTEX = "(" + OBJECT_NAME + OBJECT_VERTEX_TYPE + ")";

    public static String getAll(String text) {
        return getGroup("^(The" + COMPONENT + "?" + ")", text, 0);
    }
    ...
    public static String getObject(String text) {
        return getGroup("^The" + COMPONENT + OBJECT + "?", text, 4);
    }

    public static String getObjectEdgeType(String text) {
        return getGroup("^The" + COMPONENT + OBJECT_EDGE + "?", text, 6);
    }
    ...
    public static String getObjectVertexType(String text) {
        return getGroup("^The" + COMPONENT + OBJECT_VERTEX + "?", text, 6);
    }
